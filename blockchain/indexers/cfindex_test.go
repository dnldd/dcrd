...
func TestCfIndex(t *testing.T) {
	dbPath, err := ioutil.TempDir("", "test_cfindex")
	if err != nil {
	...
	idx, err := NewCfIndex(ctx, db, chain, chainParams, subber)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Ensure index has bk5's cfilter.
	cf, err := idx.FilterByBlockHash(bk5.Hash(), wire.GCSFilterRegular)
	if err != nil {
		t.Fatal(err)
	}

	if cf == nil {
		t.Fatalf("expected the filter for %s to be indexed", bk5.Hash().String())
	}
	...
	// Ensure the index no longer has bk5's cfilter indexed after its
	// disconnection.
	cf, err = idx.FilterByBlockHash(bk5.Hash(), wire.GCSFilterRegular)
	if err != nil {
		t.Fatal(err)
	...
	if cf != nil {
		t.Fatalf("expected no indexed filter for %s", bk5.Hash().String())
	}
